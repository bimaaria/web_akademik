                                ...
                                <table class="table table-hover">
                                    ...
                                    <tbody>
                                        @foreach ($data_siswa as $siswa)
                                            <tr>
                                                <td><a href="/siswa/{{ $siswa->id }}/profil">{{ $siswa->nama_depan }}</td></a>
                                                <td><a href="/siswa/{{ $siswa->id }}/profil">{{ $siswa->nama_belakang }}</td></a>
                                                <td>{{ $siswa->jenis_kelamin }}</td>
                                                <td>{{ $siswa->agama }}</td>
                                                <td>{{ $siswa->alamat }}</td>
                                                <td>
                                                    <a href="/siswa/{{ $siswa->id }}/edit" class="btn btn-warning btn-sm">Edit</a>
                                                    <a href="/siswa/{{ $siswa->id }}/delete" class="btn btn-danger btn-sm" onclick="return confirm('Yakin Menghapus Data?')">Hapus</a>
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
            ...
            <div class="modal-body">
                <form action="/siswa/create" method="POST">
                    ...
                    <div class="form-group{{ $errors->has('nama_depan') ? 'has-error' : '' }}">
                        ...
                        <input name="nama_depan" type="text" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Masukkan nama depan" value="{{ old('nama_depan') }}">
